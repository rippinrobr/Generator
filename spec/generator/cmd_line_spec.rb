...
def check_for_print_usage(output)
  ...
  output.should_receive(:puts).with('--imports, -imp   the name of the libraries to include in the generated source')
  output.should_receive(:puts).with('--model, -m  src - indicates how you want the model code created')
  ...
  describe CmdLine do
    ...
    describe "Missing/Invalid Optional Parameter values" do
      it "should return the model's valid values when invalid value is passed in (-m)" do
        ...
        output.should_receive(:puts).with("Supported Output Options: src (default)")
