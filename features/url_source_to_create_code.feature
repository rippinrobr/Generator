Feature: Create Code from a URL Source
  ...
  Scenario Outline:  Read data from a URL that returns JSON
    ...
    And I have a model model class name of "<model_class_name>"
    ...
    Scenarios:  valid options
      | url | language | model_class_name | model_file_name | model_output_dir | service_file_name | service_output_dir | model_output |
      | http://localhost:8098/riak/era_percentile/1979_AL | ruby | seasonal_era_percentile | seasonal_era_percentile.rb | /tmp | seasonal_era_percentile_service.rb | /tmp | src |
      | http://localhost:8098/riak/era_percentile/1979_AL | c_sharp | SeasonalEraPercentile | SeasonalEraPercentile.cs | /tmp | SeasonalEraPercentileService.cs | /tmp | src |
